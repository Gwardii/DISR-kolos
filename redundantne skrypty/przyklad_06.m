...
for k = 1:N+1
    ...
    H(k) = goalFunction(q, qMax, qMin);
    gamma = taskGain(t, Tend, gammaMax);
    dphidt = - gamma * goalFunctionGradient(q, qMax, qMin);
...
function H = goalFunction(q, qMax, qMin)
...
H = 0.5 * sum(((q - 0.5 * (qMax + qMin)) ./ (qMax - qMin)).^2);

end

function dHdq = goalFunctionGradient(q, qMax, qMin)
...
dHdq = ( q - 0.5 * (qMax + qMin)) ./ (qMax - qMin).^2;
